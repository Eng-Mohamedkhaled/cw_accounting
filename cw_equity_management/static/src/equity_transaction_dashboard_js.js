...
class EquityTransactionDashboardJS extends Component {
    setup() {
        ...
        onMounted(async () => {
            ...
            await this.loadReport();    // Then load the report
        });
    }
    ...
    async getReportData(options) {
        // Use the session info to make an RPC call to get report data
        try {
            // Use the direct RPC call approach with proper formatting
            const result = await rpc("/web/dataset/call_kw", {
                model: 'equity.transaction',
                method: 'get_dashboard_data',
                args: [
                    [], // empty recordset (dummy_records parameter)
                    options.date_from || null,
                    options.date_to || null,
                    options.partner_id || null,
                    options.transaction_type || null,
                    this.sessionInfo.user_companies.current_company || null
                ],
                kwargs: {}
            });
            ...
            return { transactions: [], equity_info: [] };
        }
    }
    ...
    async loadReport() {
        ...
        try {
            ...
            // Create a new method to get the report data directly
            const reportData = await this.getReportData(options);

            // Update state with the report data
            this.state.transactions = reportData.transactions || [];
            this.state.equityInfo = reportData.equity_info || [];
    ...
    formatCurrency(amount) {
        ...
        if (typeof session !== 'undefined' && session.user_context && session.user_context.lang) {
            try {
                return new Intl.NumberFormat(session.user_context.lang.replace('_', '-')).format(amount);
            } catch (e) {
...
EquityTransactionDashboardJS.template = "cw_equity_management.EquityTransactionDashboardJS";
EquityTransactionDashboardJS.components = { Layout, Dropdown };
registry.category("actions").add("cw_equity_management.equity_transaction_dashboard_js", EquityTransactionDashboardJS);

export default EquityTransactionDashboardJS;